...
def _get_bbox_vote_mask(preds, h, w, ref_tensor):
    """Return the bbox/depth-valid mask used for voting.

    ``vote_valid_mask`` is derived solely from the detector crop and
    inference-available depth.  It is deliberately not a GT segmentation
    mask.  ``pose_mask`` remains a legacy fallback for older architectures.
    """
    b = ref_tensor.size(0)
    if 'vote_valid_mask' in preds:
        mask = preds['vote_valid_mask']
    elif 'pose_mask' in preds:
        mask = preds['pose_mask']
    elif 'bbox_mask' in preds:
        mask = preds['bbox_mask']
    elif 'mask' in preds:
        mask = preds['mask']
    else:
        return torch.ones((b, h * w), dtype=torch.bool, device=ref_tensor.device)

    mask = mask.detach().float().to(ref_tensor.device)
    if mask.dim() == 3:
    ...
    return (mask.squeeze(1).view(b, -1) > 0.5)
...
def close_logger(logger):
    """Close and detach handlers so epoch loggers cannot accumulate."""
...
def save_pred_and_gt_json(rt_list, total_instance_list, gt_rt_list, gt_cls_list, save_path, time_list=None):
    rt = np.stack(rt_list)
    gt = np.stack(gt_rt_list)
    cls = np.stack(gt_cls_list)
    ...
    # Default all times to -1 when no list is provided
    if time_list is None:
        time_list = [-1] * len(total_instance_list)
    ...
    for i in range(len(total_instance_list)):
        ...
        obj_id_ls.append(cls.tolist()[i][0])
        score_ls.append(1)
        time_ls.append(time_list[i])  # use the provided time
...
def post_processing_ycb_quaternion(preds, sym_list=[]):
    ...
    mask = _get_bbox_vote_mask(preds, h, w, px)
    res_T = []
    for i in range(b):
        valid_pixels = mask[i].nonzero().view(-1)
        num_val = valid_pixels.size()[0]
...
def post_processing_ycb_quaternion_wi_vote(preds, sym_list=[]):
    ...
    for i in range(b):
        ...
        # res_px[3, 32]
        weight_sum = torch.sum(s_v).clamp_min(1e-6)
        res_t = torch.sum(s_v * res_t, dim=1) / weight_sum
        res_q = torch.sum(s_v * res_q, dim=1) / weight_sum
        res_q = res_q / torch.norm(res_q, dim=0).clamp_min(1e-6)
...
def post_processing_sarr_wi_vote(preds, sym_list=[]):
    """Decode confidence-weighted SARR votes into final [R|t] transforms."""
    cls_ids = preds['cls_id']
    b, channels, h, w = preds['pred_r'].size()
    if channels != 6:
        raise ValueError('SARR post-processing expects six rotation channels')

    pred_sarr = preds['pred_r'].detach().view(b, 6, -1)
    global_sarr = preds.get('global_sarr')
    if global_sarr is not None:
        global_sarr = global_sarr.detach()
    pred_t = preds['pred_t'].detach().view(b, 3, -1)
    pred_score = preds['pred_s'].detach().view(b, -1)
    mask = _get_bbox_vote_mask(preds, h, w, pred_sarr)
    transforms = []

    for i in range(b):
        valid_pixels = mask[i].nonzero().view(-1)
        if valid_pixels.numel() < 32:
            valid_pixels = torch.arange(h * w, device=pred_sarr.device)

        scores = pred_score[i, valid_pixels]
        top = torch.topk(scores, min(valid_pixels.numel(), 32), largest=True)
        selected = valid_pixels[top.indices]
        weights = top.values
        weight_sum = weights.sum().clamp_min(1e-6)

        if global_sarr is not None:
            # The redesigned network predicts one object-level rotation.  Do
            # not let the translation confidence weights alter that rotation.
            sarr_vote = F.normalize(global_sarr[i], dim=0, eps=1e-6)
        else:
            # Compatibility path for dense SARR models. Cosine regression
            # leaves global magnitude unconstrained, so normalize each vote.
            selected_sarr = pred_sarr[i, :, selected]
            selected_sarr = F.normalize(selected_sarr, dim=0, eps=1e-6)
            sarr_vote = torch.sum(
                selected_sarr * weights.unsqueeze(0), dim=1) / weight_sum
        translation = torch.sum(
            pred_t[i, :, selected] * weights.unsqueeze(0), dim=1) / weight_sum
        rotation = sarr_to_matrix(sarr_vote, cls_ids[i])
        transforms.append(torch.cat((rotation, translation.view(3, 1)), dim=1))

    return torch.stack(transforms, dim=0)

def post_processing_tless_quaternion_wi_vote(preds, sym_list=[]):
    '''
    get final transform matrix T=[R|t] and confidence scores from prediction results with mask
    :param preds: output of pose net ['pred_x'][bs, 4, h, w]...
    :return: (T, confs) where T is [bs, 3, 4], confs is [bs] float tensor
    '''
    cls_ids = preds['cls_id']
    b, c, h, w = preds['pred_r'].size()
    px = preds['pred_r'].detach().view(b, 4, -1)
    ...
    ps = preds['pred_s'].detach().view(b, -1)   # confidence score per pixel

    mask = _get_bbox_vote_mask(preds, h, w, px)

    res_T = []
    res_conf = []                     # confidence per batch element
    ...
    for i in range(b):
        ...
        # Take the top-K highest-scoring pixels
        k_s = torch.topk(s, min(num_val, 32), dim=0, largest=True)
        s_id = k_s.indices.unsqueeze(dim=0).repeat(4, 1)
        s_v = k_s.values                # top-K scores
        res_t = torch.gather(t, dim=1, index=s_id[:3, :])
        res_q = torch.gather(q, dim=1, index=s_id[:4, :])

        # Unify quaternion signs (avoid cancellation)
        n_id = res_q[0, :] < 0
        res_q[:, n_id] = -res_q[:, n_id]
        ...
        # ---------- Per-pose confidence ----------
        # Confidence = mean of the top-K scores (reflects vote concentration better than the max)
        conf_i = torch.mean(s_v).item()
        res_conf.append(conf_i)

    # Return the pose matrices and confidence tensor
    return torch.stack(res_T, dim=0), torch.tensor(res_conf, device='cuda')
